...
class Session {
	...
	public function loginTo($u, $pr, $rp){
		
		$prepared = $this->prepare("SELECT count(*) as c FROM invento_question WHERE username=? && preg_1=? && preg_2=?", 'isLoggedTo()');
		$this->bind_param($prepared->bind_param('sss', $u, $pr, $rp), 'loginTo()');
		$this->execute($prepared, 'loginTo()');
		...
		$_SESSION['invento_logged']['u'] = $u;			
		$_SESSION['invento_logged']['pr'] = $pr;
		$_SESSION['invento_logged']['rp'] = $rp;

		return true;

	}
	public function isLoggedTo(){

		
		if(!isset($_SESSION['invento_logged']) || !is_array($_SESSION['invento_logged']))
			return false;
		
		if(!isset($_SESSION['invento_logged']['u']) || !isset($_SESSION['invento_logged']['pr']) || !isset($_SESSION['invento_logged']['rp']))
			return false;

		$u =  $_SESSION['invento_logged']['u'];
		$pr = $_SESSION['invento_logged']['pr'];
		$rp = $_SESSION['invento_logged']['rp'];
		
		$prepared = $this->prepare("SELECT count(*) as c FROM invento_question WHERE username=? && preg_1=? && preg_2=?", 'isLoggedTo()');
		$this->bind_param($prepared->bind_param('sss', $u, $pr, $rp), 'isLoggedTo()');
		$this->execute($prepared, 'isLoggedTo()');
