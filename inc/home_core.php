...
class Home {
	...
	public function general_registered_pays() {
		$res = $this->query("SELECT COUNT(*) as c FROM invento_pays", 'general_registered_pays()');
		$obj = $res->fetch_object();
		return $obj->c;
	}
	...
	public function general_warehouse_value_pays() {
		$res = $this->query("SELECT SUM(monto) as s FROM invento_pays", 'general_warehouse_value_pays()');
		$obj = $res->fetch_object();
		if($obj->s == '')
			return 0;
		return $this->parse_cost($obj->s);
